...
func dataStorage(data Data, id string) {
	...
	db.Model(&models.GoodsItem{}).Where("goods_id = ?", id).Find(&judge)
	if strconv.Itoa(judge.GoodsId) != id {
		//如果已经存在此商品就删除掉原来的
		if judge.Itemid == goodsInform.Itemid {
			m.Lock()
			db.Delete(models.GoodsItem{}, "itemid = ?", judge.Itemid)
			m.Unlock()
		}
		m.Lock()
